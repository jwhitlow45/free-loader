...
const Sidebar: React.FunctionComponent = () => {
  ...
  const loadGames = useCallback(async (retries = 0) => {
    ...
    if (response.success) {
      ...
      if (retries <= MAX_RETRIES) {
        await PyCaller.loggerError('Failed to read deals from json db...retrying');
        loadGames(retries + 1);
      } else {
        await PyCaller.loggerError(`Reached max retry limit of ${MAX_RETRIES}...cannot load page.`);
  ...
  return (
    <div>
      <ActionsPanel loadGamesFunction={ loadGames } />
