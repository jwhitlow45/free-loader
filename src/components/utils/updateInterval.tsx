...
export async function updateInterval(): Promise<NodeJS.Timer> {
  ...
  await PyCaller.logger(`Next games list update at ${new Date(new Date().getTime() + freq_ms)}`);
  let timer = setInterval(async () => {
    ...
    await PyCaller.logger(`Next games list update at ${new Date(now.getTime() + freq_ms)}`);
  }, freq_ms)
